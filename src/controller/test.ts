...
export function testCtrl(scope: Scope): Promise<HttpResponse> {
  ...
  if (reqBody.text.includes('pancakes')) {
    return fetch(`${BASE_URL}/bots/post`, {
      method: 'POST',
      body: JSON.stringify({bot_id: scope.config.test_bot_id, text: random(quotes)})
    })
    .then((res) => {
  ...
  const resBody = {bot_id: scope.config.test_bot_id, text: `${matches[1]}`}
  return fetch(`${BASE_URL}/bots/post`, {
    method: 'POST',
